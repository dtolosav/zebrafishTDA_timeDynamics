...
function [] = getDMtotxt(path_to_file, sim_number)
if ~exist('path_to_file','var')
    % if parameter does not exist, default it to something
    try
	    path_to_file = "Raw_Data/WT_default/Out_WT_default_1";
	catch
		disp(['Could not find file']);
	end
end
% load all the cell coordinates of all cell types at each time, from 1 to 46
try
	load(path_to_file);
catch
	disp(['Could not find file: ',path_to_file]);
end
cd("Distance_Matrices/WT");
