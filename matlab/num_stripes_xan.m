...
for j = 2: 1 : 46
    num_stripes(j) = length(find(barcodes_xan_betti1{j}(:,1) < 150 & barcodes_xan_betti1{j}(:,2) > 210));
end

num_stripes

%___________________________SOME SCATTERPLOTS TO CHECK IF THE STRIPE COUNT IS GOOD ____________

sample_times =[2,3,4,5,6,7,8,9,10,20,30,40];
for i = 1 : 1: 12
    h(i) = figure;
    caption1= 'Pattern at time ';
    caption2= '. Number of stripes =';
    temp_cells=cells_xan{sample_times(i)};
    temp_time=sample_times(i);
    scatter(temp_cells(:,1),temp_cells(:,2));
    title(sprintf("%s %d %s %d", caption1 ,sample_times(i), caption2, num_stripes(temp_time)));
    axis equal;
    figname=strcat("Counting Stripes\WT_808\Xan\150-210\Stripe_Count_WT_808_Xan_150-210_time",string(sample_times(i)));
    savefig(h(i),figname);
end
savefig(h,"Counting Stripes\WT_808\Xan\150-210\Allfigs");