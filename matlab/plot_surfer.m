...
% coordinate data of an agent-based model for zebrafish in the form of a matrix and plots 3D surfaces 
% for betti 0 and 1 as functions of epsilon and time.
%
% Inputs:number of simulation; 
% 
% Outputs: 2 surf plots.
...
function [handle] = plot_surfer(sim_number)
...
surfc(eps_values_b0 ,time_values, betti0);
...
surfc(eps_values_b1 ,time_values, betti1);
caption = 'Betti(time,eps) of dimension ';
...
sample_times =[3,10,20,30,46];
for i =1 : 5
    figure;
    caption= 'Pattern at time ';
    temp_cells=cells_mel{sample_times(i)};
    scatter(temp_cells(:,1),temp_cells(:,2));
    title(sprintf("%s %d", caption ,sample_times(i)));
end
